...
form.addEventListener("submit", (e) => {
    ...
    let n_id = 0; // todo id값
    let dateComp = duedate.value.substring(0, 4) +
        "-" +
    ...
    try {
        ...
        const todos = new Todos(n_id.toString(), convertToCategory(category.value), title.value, parseInt(duedate.value), false);
...
Array.from(delBtn).forEach((btn) => {
    btn.addEventListener("click", (e) => {
        const id = btn.id.substring(11); // button.setAttribute("id", "delete-btn-" + i);
        const todoList = localStorage.getItem("todoList");
        let todoArray = [];
        todoArray = JSON.parse(todoList);
